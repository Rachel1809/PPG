...
class MeshEvaluator(object):
    ...
    def eval_mesh(self, mesh, pointcloud_tgt, normals_tgt, thresholds=np.linspace(1./1000, 1, 1000)):
        ...
        if len(mesh.vertices) != 0 and len(mesh.faces) != 0:
            pointcloud, idx = trimesh.sample.sample_surface_even(mesh, self.n_points)
    ...
    def eval_pointcloud(self, pointcloud, pointcloud_tgt,
        ...
        #Hausdorff distance
        hausdorff = distance_hausdorff(pointcloud, pointcloud_tgt)
...
def distance_hausdorff(points_src, points_tgt):
    dist_src_tgt, _, _ = spatial.distance.directed_hausdorff(points_src, points_tgt)
    dist_tgt_src, _, _ = spatial.distance.directed_hausdorff(points_tgt, points_src)
    dist = max(dist_src_tgt, dist_tgt_src)
    return dist
